...
//
// CHANGES:
//  - FREE_CAP (10 questions) enforced for unpaid users
//  - Upgrade banner shown in hub
...
export default function EntranceExamDailyMockHub() {
  ...
  const subject = location.state?.subject ?? {
    id: 'entrance_general', label: 'Entrance Exam', shortLabel: 'Entrance Exam', icon: '🎓', color: '#0D9488',
  };
  ...
  useEffect(() => {
    ...
    setSessLoading(true);
    getDocs(query(collection(db, 'entranceExamSessions'), where('userId', '==', currentUser.uid), where('examType', '==', 'entrance_daily_mock'), where('subject', '==', subject.id)))
      .then(snap => {
        const results = snap.docs.map(d => ({ id: d.id, ...d.data() }));
        results.sort((a, b) => (a.completedAt?.toDate?.()?.getTime?.() || 0) < (b.completedAt?.toDate?.()?.getTime?.() || 0) ? 1 : -1);
        setSessions(results);
      })
      .catch(() => setSessions([]))
  ...
  if (phase === 'intro') {
    return (
      <div style={{ padding: '24px', maxWidth: 700 }}>
        <button onClick={() => navigate('/entrance-exam')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--teal)', fontWeight: 700, fontSize: 13, padding: 0, marginBottom: 20, display: 'flex', alignItems: 'center', gap: 6 }}>
        ...
        <div style={{ background: 'var(--bg-card)', border: '1.5px solid var(--border)', borderRadius: 20, padding: '32px 28px', textAlign: 'center', marginBottom: 24 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 12, marginBottom: 24 }}>
            {[
              ...
              <div key={label} style={{ background: 'var(--bg-tertiary)', borderRadius: 12, padding: '14px 12px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                <span style={{ fontSize: 22 }}>{icon}</span>
                <span style={{ fontSize: 11, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 0.5 }}>{label}</span>
                <span style={{ fontSize: 16, fontWeight: 700, color: 'var(--text-primary)', fontFamily: mono ? 'monospace' : 'inherit' }}>{value}</span>
              </div>
            ))}
          </div>
          ...
          {!isPaid && (
            <div style={{ background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.25)', borderRadius: 10, padding: '12px 14px', marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
              <span style={{ fontSize: 13, fontWeight: 700, color: '#F59E0B' }}>⚡ Free preview — capped at {FREE_CAP} questions</span>
              <button onClick={() => navigate('/entrance-exam/payment')} style={{ padding: '5px 14px', borderRadius: 20, border: '1.5px solid #F59E0B', background: 'transparent', color: '#F59E0B', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>Upgrade →</button>
            </div>
          )}
          <button onClick={() => setPhase('hub')} style={{ width: '100%', padding: '14px', background: 'var(--teal)', border: 'none', borderRadius: 12, color: '#fff', fontFamily: 'inherit', fontWeight: 800, fontSize: 16, cursor: 'pointer', letterSpacing: 0.3 }}>
            🚀 Proceed to Exam
          </button>
        </div>

        {sessions.length > 0 && (
          <div style={{ background: 'var(--bg-card)', border: '1.5px solid var(--border)', borderRadius: 16, padding: '20px 24px' }}>
            <div style={{ fontWeight: 700, fontSize: 14, color: 'var(--text-primary)', marginBottom: 14, textTransform: 'uppercase', letterSpacing: 0.5 }}>📋 Your Recent Attempts</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {sessions.slice(0, 5).map(session => {
                const pct = session.scorePercent ?? Math.round(((session.correct || 0) / (session.totalQuestions || 1)) * 100);
                const passed = pct >= PASS_MARK;
                const date = session.completedAt?.toDate ? session.completedAt.toDate() : session.completedAt ? new Date(session.completedAt) : null;
                const dateStr = date ? date.toLocaleDateString('en-NG', { day: '2-digit', month: 'short', year: 'numeric' }) : '—';
                return (
                  <div key={session.id} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    <span style={{ width: 100, fontSize: 12, color: 'var(--text-muted)' }}>{dateStr}</span>
                    <div style={{ flex: 1, height: 8, background: 'var(--bg-tertiary)', borderRadius: 4, overflow: 'hidden' }}>
                      <div style={{ height: '100%', width: `${pct}%`, background: passed ? '#16A34A' : '#EF4444', borderRadius: 4, transition: 'width 0.4s ease' }} />
                    </div>
                    <span style={{ width: 40, fontSize: 13, fontWeight: 700, textAlign: 'right', color: passed ? '#16A34A' : '#EF4444' }}>{pct}%</span>
                  </div>
                );
              })}
            </div>
  ...
  return (
    <div style={{ padding: '24px', maxWidth: 860 }}>
      <button onClick={() => setPhase('intro')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--teal)', fontWeight: 700, fontSize: 13, padding: 0, marginBottom: 20, display: 'flex', alignItems: 'center', gap: 6 }}>← Back</button>

      <div style={{ display: 'inline-flex', alignItems: 'center', gap: 10, background: `${subject.color}18`, border: `1.5px solid ${subject.color}40`, borderRadius: 40, padding: '8px 16px', marginBottom: 28 }}>
        <span style={{ fontSize: 20 }}>{subject.icon}</span>
        <div>
          <div style={{ fontSize: 11, color: subject.color, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5 }}>DAILY MOCK</div>
          <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-primary)' }}>{subject.label}</div>
        </div>
      </div>

      <div style={{ background: 'var(--bg-card)', border: '2px solid var(--teal)', borderRadius: 18, padding: 24, marginBottom: 32, boxShadow: '0 0 0 4px rgba(13,148,136,0.06)' }}>
        ...
        {/* Free preview banner inside hub card */}
        {!isPaid && (
          <div style={{ background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.3)', borderRadius: 10, padding: '10px 14px', marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10 }}>
            <span style={{ fontSize: 13, fontWeight: 700, color: '#F59E0B' }}>⚡ Free preview — capped at {FREE_CAP} questions</span>
            <button onClick={() => navigate('/entrance-exam/payment')} style={{ padding: '5px 14px', borderRadius: 20, border: '1.5px solid #F59E0B', background: 'transparent', color: '#F59E0B', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>Upgrade →</button>
          </div>
        )}
      ...
      <div>
        <div style={{ fontWeight: 800, fontSize: 15, color: 'var(--text-primary)', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
          📋 Previous Exams
          {sessions.length > 0 && <span style={{ fontSize: 12, background: 'var(--bg-tertiary)', color: 'var(--text-muted)', padding: '2px 10px', borderRadius: 20, fontWeight: 700 }}>{sessions.length}</span>}
        </div>
        {sessLoading ? (
          <div style={{ textAlign: 'center', padding: '40px 0' }}><div className="spinner" style={{ width: 32, height: 32, margin: '0 auto' }} /><div style={{ color: 'var(--text-muted)', fontSize: 13, marginTop: 10 }}>Loading exam history…</div></div>
        ) : sessions.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '40px 20px', background: 'var(--bg-card)', borderRadius: 14, border: '1.5px dashed var(--border)' }}>
            <div style={{ fontSize: 36, marginBottom: 10 }}>📭</div>
            <div style={{ fontWeight: 700, color: 'var(--text-primary)', marginBottom: 4 }}>No exams taken yet</div>
            <div style={{ fontSize: 13, color: 'var(--text-muted)' }}>Take your first mock above — it'll appear here when done.</div>
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {sessions.map(session => {
              ...
              return (
                <div key={session.id} style={{ background: 'var(--bg-card)', border: '1.5px solid var(--border)', borderLeft: `4px solid ${passed ? '#16A34A' : '#EF4444'}`, borderRadius: 14, padding: '16px 18px', display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
                  <div style={{ width: 54, height: 54, borderRadius: 12, flexShrink: 0, background: passed ? 'rgba(22,163,74,0.1)' : 'rgba(239,68,68,0.1)', border: `2px solid ${passed ? 'rgba(22,163,74,0.3)' : 'rgba(239,68,68,0.3)'}`, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                    <div style={{ fontSize: 15, fontWeight: 900, color: passed ? '#16A34A' : '#EF4444', lineHeight: 1 }}>{pct}%</div>
                    <div style={{ fontSize: 9, fontWeight: 700, color: passed ? '#16A34A' : '#EF4444', textTransform: 'uppercase', letterSpacing: 0.5 }}>{passed ? 'PASS' : 'FAIL'}</div>
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontWeight: 700, fontSize: 14, color: 'var(--text-primary)', marginBottom: 4 }}>{session.examName || 'Daily Mock'}</div>
                    <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap' }}>
                      <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>📅 {dateStr}{timeStr ? ` · ${timeStr}` : ''}</span>
                      <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>✅ {session.correct ?? '?'} / {session.totalQuestions ?? '?'} correct</span>
                    </div>
                  </div>
                  <button className="btn btn-ghost btn-sm" onClick={() => handleReview(session)} style={{ flexShrink: 0, fontWeight: 700 }}>🔍 Review</button>
                </div>
              );
            })}
          </div>
