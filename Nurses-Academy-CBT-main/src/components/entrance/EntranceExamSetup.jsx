...
//
// FIX: navigates to /entrance-exam/session (EntranceExamSession) instead of
//      /exam/session (NMCN ExamSession). This ensures school past questions
//      are saved to entranceExamSessions and appear in EntranceAnalysis.
...
function ToggleRow({ label, desc, checked, onChange }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
      <div>
        <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-primary)', fontFamily: F }}>{label}</div>
        <div style={{ fontSize: 12, color: 'var(--text-muted)', fontFamily: F }}>{desc}</div>
      </div>
...
export default function EntranceExamSetup() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        // ── Fetch all questions for this school ──────────────────────────
        // Try schoolId first (new format), fall back to schoolName field
        let qSnap;
        try {
        ...
        if (user) {
          try {
            const attSnap = await getDocs(query(
              ...
              where('examType', '==', 'entrance_school_exam'),
              limit(5),
            ));
            const attempts = attSnap.docs.map(d => ({ id: d.id, ...d.data() }));
            // filter to this school
            const filtered = attempts.filter(a =>
              a.schoolId === school.id || a.schoolName === school.name
            );
            filtered.sort((a, b) =>
              (b.completedAt?.toMillis?.() || 0) - (a.completedAt?.toMillis?.() || 0)
            );
            setPrevAttempts(filtered.slice(0, 5));
          } catch (e) { console.warn('Prev attempts load:', e.message); }
        }
  ...
  const handleStart = () => {
    ...
    // ── FIXED: navigate to /entrance-exam/session (EntranceExamSession)
    //          NOT /exam/session (NMCN ExamSession)
    // EntranceExamSession will load questions from entranceExamQuestions
    // and save results to entranceExamSessions — captured by EntranceAnalysis
    navigate('/entrance-exam/session', {
      state: {
        // schoolMode flag tells EntranceExamSession to load by schoolId
        schoolMode:         true,
        poolMode:           false,
        schoolId:           school.id,
        schoolName:         school.name,
        examYear:           year || 'all',
        examType:           'entrance_school_exam',
        examName:           `${school.shortName || school.name}${year && year !== 'all' ? ` — ${year}` : ' — Past Questions'}`,
        subject:            school.id,      // used for grouping in analysis
        count,
        doShuffle:          shuffle,
        timeLimitMin:       mode === 'practice' ? 0 : timeLimit,
      },
    });
  };
  ...
  const HEAD = { fontWeight: 700, fontSize: 14, color: 'var(--text-primary)', marginBottom: 4, fontFamily: F };

  return (
    <div style={{ padding: '24px 16px', maxWidth: 720, margin: '0 auto', fontFamily: F, color: 'var(--text-primary)' }}>
      ...
      <div style={{
        ...
        <div>
          <div style={{ fontSize: 11, color: 'var(--teal)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5, fontFamily: F }}>Selected School</div>
          <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-primary)', fontFamily: F }}>{school.name}</div>
        </div>
      </div>

      <h2 style={{ fontFamily: H, fontWeight: 900, margin: '0 0 4px', color: 'var(--text-primary)', fontSize: 'clamp(1.4rem,3vw,2rem)' }}>
        ⚙️ Set Up Your Exam
      </h2>
      <p style={{ color: 'var(--text-muted)', fontSize: 13, margin: '0 0 24px', fontFamily: F, fontWeight: 700 }}>
        {school.state ? `📍 ${school.state}` : ''} {school.state ? '·' : ''} {allCount > 0 ? `${allCount} questions available` : 'Loading questions…'}
      </p>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div className="card" style={SEC}>
          <div style={HEAD}>
            📅 Exam Year
            {availableYears.length > 0 && <span style={{ fontWeight: 400, fontSize: 12, color: '#EF4444', marginLeft: 8, fontFamily: F }}>* required</span>}
          </div>
          {loadingMeta ? (
            <p style={{ color: 'var(--text-muted)', fontSize: 13, fontFamily: F }}>Loading available years…</p>
          ) : availableYears.length === 0 ? (
            <p style={{ color: 'var(--text-muted)', fontSize: 13, fontFamily: F }}>
              Questions not yet uploaded for this school. Check back soon!
            </p>
          ) : (
            <>
              <p style={{ fontSize: 12, color: 'var(--text-muted)', margin: '4px 0 12px', fontFamily: F, fontWeight: 700 }}>
        ...
        <div className="card" style={SEC}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 8 }}>
            {[
              ...
              { id: 'practice', icon: '📖', label: 'Practice Mode',   desc: 'No timer · See answers at your pace' },
            ].map(m => (
              <div
                key={m.id}
              ...
              >
                ...
                <div>
                  <div style={{ fontWeight: 700, fontSize: 13, color: 'var(--text-primary)', fontFamily: F }}>{m.label}</div>
                  <div style={{ fontSize: 11, color: 'var(--text-muted)', fontFamily: F }}>{m.desc}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 8 }}>
            {QUESTION_COUNTS.filter(n => n <= Math.max(allCount, 10)).concat(
              ...
              <button key={n} onClick={() => setCount(n)} style={{
                padding: '10px 18px', border: '2px solid', borderRadius: 8,
                cursor: 'pointer', fontSize: 13, fontWeight: 700, fontFamily: F, transition: 'all 0.2s',
        ...
        {mode === 'timed' && (
          <div className="card" style={SEC}>
            ...
            <select className="form-input form-select" value={timeLimit}
              ...
              style={{ marginTop: 8, maxWidth: 220, fontFamily: F }}>
              ...
              {prevAttempts.map(a => {
                ...
                return (
                  <div key={a.id} style={{
                    ...
                    <div>
                      <span style={{ fontWeight: 700, fontSize: 13, color: 'var(--text-primary)', fontFamily: F }}>
                        {a.examName || school.name}
                      </span>
                      <span style={{ fontSize: 11, color: 'var(--text-muted)', marginLeft: 10, fontFamily: F, fontWeight: 700 }}>
        ...
        <div className="card" style={{ ...SEC, background: 'linear-gradient(135deg, var(--bg-card), var(--bg-secondary))' }}>
          <div style={{ fontWeight: 700, color: 'var(--text-primary)', fontSize: 14, marginBottom: 14, fontFamily: F }}>
            👁️ Exam Preview
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 16 }}>
            {[
              ...
              <div key={k} style={{ background: 'var(--bg-tertiary)', borderRadius: 10, padding: '10px 12px' }}>
                <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 2, fontFamily: F }}>{k}</div>
                <div style={{
          ...
          <button
            ...
            style={{
              fontSize: 16, padding: '14px', fontFamily: F, fontWeight: 800,
              opacity: (availableYears.length > 0 && !year) ? 0.5 : 1,
            }}
          >
            🚀 Start Exam
          </button>
