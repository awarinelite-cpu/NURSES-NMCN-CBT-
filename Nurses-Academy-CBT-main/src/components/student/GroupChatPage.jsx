// src/components/student/GroupChatPage.jsx
// Route: /group-chat/:subjectId
// Subject-based group chat with flagged/bookmarked question insertion,
// swipe-to-reply, and debate threading.
...
/* ─── Subject groups metadata ────────────────────────────── */
const GROUP_META = {
  general_nursing:    { label: 'General Nursing (RN)',        icon: '🏥', color: '#0D9488' },
  midwifery:          { label: 'Midwifery',                   icon: '👶', color: '#7C3AED' },
  mental_health:      { label: 'Psychiatric Nursing',         icon: '🧠', color: '#2563EB' },
  public_health:      { label: 'Public Health Nursing',       icon: '🌍', color: '#16A34A' },
  perioperative:      { label: 'Theatre Nursing',             icon: '🔪', color: '#DC2626' },
  orthopaedic:        { label: 'Orthopaedic Nursing',         icon: '🦴', color: '#D97706' },
  ophthalmic:         { label: 'Ophthalmic Nursing',          icon: '👁️', color: '#0891B2' },
  paediatric:         { label: 'Paediatric Nursing',          icon: '🧒', color: '#F59E0B' },
  accident_emergency: { label: 'A&E Nursing',                 icon: '🚨', color: '#EF4444' },
  icu_critical:       { label: 'ICU / Critical Care',         icon: '💉', color: '#6366F1' },
  community_health:   { label: 'Community Health Nursing',    icon: '🏘️', color: '#10B981' },
  anatomy_physiology: { label: 'Anatomy & Physiology',        icon: '🫀', color: '#F97316' },
  pharmacology:       { label: 'Pharmacology',                icon: '💊', color: '#8B5CF6' },
  microbiology:       { label: 'Microbiology & Infection',    icon: '🦠', color: '#06B6D4' },
  medical_surgical:   { label: 'Medical-Surgical Nursing',    icon: '⚕️', color: '#059669' },
  exam_tips:          { label: 'NMCN Exam Tips & Strategies', icon: '📝', color: '#CA8A04' },
  study_lounge:       { label: 'General Study Lounge',        icon: '☕', color: '#78716C' },
};
...
function QuestionCard({ question, accentColor, compact = false }) {
            ...
            return (
              <div key={i} style={{
                ...
                <span style={{
                  ...
                  fontSize: 12, color: isCorrect ? '#34D399' : '#CBD5E1',
                  lineHeight: 1.5,
                }}>{optText}</span>
          ...
          >
            {isQuestion ? (
              <>
                ...
                {/* User comment on the question */}
                {msg.text && (
                  <div style={{
                    marginTop: 8, padding: '6px 10px',
                    background: 'rgba(255,255,255,0.06)',
                    borderRadius: 8, borderLeft: `3px solid ${accentColor}`,
                    fontFamily: F, fontWeight: 700, fontSize: 12, color: '#CBD5E1',
                    lineHeight: 1.55,
                  }}>
                    💬 {msg.text}
                  </div>
            ...
            <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginTop: 4 }}>
              {Object.entries(msg.reactions).map(([emoji, count]) => count > 0 && (
                <span key={emoji} style={{
                  ...
                  fontSize: 12, fontFamily: F, fontWeight: 700, color: '#E9EDEF',
                  cursor: 'pointer',
...
function FlaggedQuestionPicker({ userId, onSelect, onClose, accentColor }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      try {
        // Load both bookmarks and fetch their question details
        ...
        const enriched = await Promise.all(
          bms.map(async bm => {
            try {
              ...
              if (!qSnap.empty) {
                const qData = { id: bm.questionId, ...qSnap.docs[0].data() };
                return { ...bm, question: qData };
              }
            } catch {}
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* If question selected — show comment input */}
        {selected ? (
          <div style={{ flex: 1, overflowY: 'auto', padding: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={{
              fontFamily: H, fontWeight: 900, fontSize: 13, color: accentColor, marginBottom: 4,
            ...
            <div style={{ marginTop: 8 }}>
              <div style={{ fontFamily: F, fontWeight: 700, fontSize: 13, color: '#8696A0', marginBottom: 6 }}>
                Add a comment or question for the group (optional):
              </div>
              <textarea
                value={comment}
                ...
                style={{
                  ...
                  padding: '10px 14px',
                  color: '#E9EDEF', fontFamily: F, fontWeight: 700, fontSize: 13,
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => setSelected(null)} style={{
                ...
                color: '#E9EDEF', fontFamily: F, fontWeight: 700, fontSize: 14,
                cursor: 'pointer',
              }}>← Back</button>
              <button onClick={() => onSelect(selected.question, comment.trim())} style={{
                ...
                color: '#fff', fontFamily: H, fontWeight: 900, fontSize: 14,
                cursor: 'pointer',
              ...
              {loading ? (
                ...
                <div style={{ textAlign: 'center', padding: 40 }}>
                  ...
                  <div style={{ color: '#8696A0', fontFamily: F, fontWeight: 700, fontSize: 14, lineHeight: 1.7 }}>
                    {search ? 'No bookmarks match your search.' : 'You have no bookmarked questions yet.\n\nBookmark questions during an exam session to access them here.'}
                  </div>
                </div>
              ) : (
...
export default function GroupChatPage() {
  ...
  const grp = GROUP_META[subjectId] || state?.group || { label: subjectId, icon: '💬', color: '#0D9488' };
  ...
  const [typing, setTyping] = useState({}); // { uid: { name, ts } }
  const [showMenu, setShowMenu] = useState(false);   // hamburger menu
  const [showPicker, setShowPicker] = useState(false); // flag question picker
  ...
  const colPath = `groupChats/${subjectId}/messages`;
  ...
  useEffect(() => {
    if (!myUid || !subjectId) return;
    setDoc(doc(db, 'groupChats', subjectId, 'members', myUid), {
      name: myName,
    ...
    // Get member count
    getDocs(collection(db, 'groupChats', subjectId, 'members'))
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      collection(db, 'groupChats', subjectId, 'typing'),
      snap => {
        const now = Date.now();
        const active = {};
        snap.docs.forEach(d => {
          ...
          if (data.isTyping && age < 5000) {
            active[d.id] = { name: data.name || 'Someone' };
          }
        });
        setTyping(active);
      }
  ...
  const updateTyping = useCallback((val) => {
    ...
    setDoc(doc(db, 'groupChats', subjectId, 'typing', myUid), {
      isTyping: val,
      name: myName,
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      await setDoc(doc(db, 'groupChats', subjectId), {
        lastMessage: trimmed,
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <button onClick={() => navigate('/group-chat')} style={{
          background: 'none', border: 'none', cursor: 'pointer',
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{
            fontSize: 11, fontFamily: F, fontWeight: 700,
            color: typingText ? '#25D366' : '#8696A0',
        ...
        <div ref={menuRef} style={{ position: 'relative' }}>
          <button
            ...
            style={{
              ...
              color: '#8696A0', display: 'flex', flexDirection: 'column',
          ...
          {showMenu && (
            <div style={{
              ...
              <button onClick={() => { setShowMenu(false); navigate('/bookmarks'); }} style={{
                display: 'flex', alignItems: 'center', gap: 12,
                width: '100%', padding: '13px 16px',
                border: 'none', cursor: 'pointer',
                background: 'transparent',
              ...
              >
                ...
                <div style={{ textAlign: 'left' }}>
                  ...
                  <div style={{ fontSize: 11, color: '#8696A0', marginTop: 1 }}>View all saved questions</div>
                </div>
              </button>
              <button onClick={() => { setShowMenu(false); navigate('/exams'); }} style={{
                display: 'flex', alignItems: 'center', gap: 12,
                width: '100%', padding: '13px 16px',
                border: 'none', cursor: 'pointer',
                background: 'transparent',
              ...
              >
                <span style={{ fontSize: 18 }}>📝</span>
                <div style={{ textAlign: 'left' }}>
                  <div style={{ fontFamily: H, fontWeight: 900, fontSize: 13 }}>Practice Exams</div>
                  <div style={{ fontSize: 11, color: '#8696A0', marginTop: 1 }}>Go to {grp.label} exam</div>
                </div>
              </button>
            </div>
          )}
      ...
      <div className="msg-area" style={{
        ...
        {loading ? (
          ...
          <div style={{
            ...
            <div style={{
              fontSize: 13, fontWeight: 700, color: '#8696A0',
              maxWidth: 300, lineHeight: 1.75,
            }}>
              This is the start of the <strong style={{ color: accentColor }}>{grp.label}</strong> group. Debate questions, share knowledge, and drop flagged questions for group discussion using the <strong style={{ color: accentColor }}>☰ menu</strong>.
            </div>
          </div>
        ) : (
          <>
