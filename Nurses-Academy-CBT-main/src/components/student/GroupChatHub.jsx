// src/components/student/GroupChatHub.jsx
// Route: /group-chat
// Hub showing all subject subgroups
...
// All subject groups — nursing categories + some extra discussion groups
const GROUP_SUBJECTS = [
  { id: 'general_nursing',    label: 'General Nursing (RN)',            icon: '🏥', color: '#0D9488' },
  { id: 'midwifery',          label: 'Midwifery',                       icon: '👶', color: '#7C3AED' },
  { id: 'mental_health',      label: 'Psychiatric Nursing',             icon: '🧠', color: '#2563EB' },
  { id: 'public_health',      label: 'Public Health Nursing',           icon: '🌍', color: '#16A34A' },
  { id: 'perioperative',      label: 'Theatre Nursing',                 icon: '🔪', color: '#DC2626' },
  { id: 'orthopaedic',        label: 'Orthopaedic Nursing',             icon: '🦴', color: '#D97706' },
  { id: 'ophthalmic',         label: 'Ophthalmic Nursing',              icon: '👁️', color: '#0891B2' },
  { id: 'paediatric',         label: 'Paediatric Nursing',              icon: '🧒', color: '#F59E0B' },
  { id: 'accident_emergency', label: 'A&E Nursing',                     icon: '🚨', color: '#EF4444' },
  { id: 'icu_critical',       label: 'ICU / Critical Care',             icon: '💉', color: '#6366F1' },
  { id: 'community_health',   label: 'Community Health Nursing',        icon: '🏘️', color: '#10B981' },
  { id: 'anatomy_physiology', label: 'Anatomy & Physiology',            icon: '🫀', color: '#F97316' },
  { id: 'pharmacology',       label: 'Pharmacology',                    icon: '💊', color: '#8B5CF6' },
  { id: 'microbiology',       label: 'Microbiology & Infection',        icon: '🦠', color: '#06B6D4' },
  { id: 'medical_surgical',   label: 'Medical-Surgical Nursing',        icon: '⚕️', color: '#059669' },
  { id: 'exam_tips',          label: 'NMCN Exam Tips & Strategies',     icon: '📝', color: '#CA8A04' },
  { id: 'study_lounge',       label: 'General Study Lounge',            icon: '☕', color: '#78716C' },
];

function Avatar({ label, icon, color, size = 48 }) {
  return (
    <div style={{
...
function GroupCard({ group, lastMsg, unread, onJoin }) {
  ...
  return (
    ...
    >
      <Avatar label={group.label} icon={group.icon} color={group.color} size={52} />
...
export default function GroupChatHub() {
  ...
  const [groupMeta, setGroupMeta] = useState({}); // { groupId: { lastMsg, unread } }

  // Listen to last-message meta for each group
  useEffect(() => {
    if (!user) return;
    const unsubs = GROUP_SUBJECTS.map(grp => {
      const metaRef = doc(db, 'groupChats', grp.id);
  ...
  const filtered = GROUP_SUBJECTS.filter(g =>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: '#8696A0', fontFamily: F, fontWeight: 700, marginTop: 2 }}>
              {GROUP_SUBJECTS.length} subject groups
              {totalUnread > 0 && (
