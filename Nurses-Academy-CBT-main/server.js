...
// ─────────────────────────────────────────────────────────────────
// POST /api/ai-tag-questions
// Suggests { courseId, topic } for a BATCH (up to 100) of untagged
// questions using Gemini. Never writes to Firestore itself — the admin
// reviews every suggestion in the UI before saving.
// Requires env var: GEMINI_API_KEY
// ─────────────────────────────────────────────────────────────────
const AI_TAG_MAX_BATCH = 100;

function buildTagPrompt({ questions, courses }) {
...
app.post('/api/ai-tag-questions', async (req, res) => {
  const GEMINI_KEY = process.env.GEMINI_API_KEY;
  if (!GEMINI_KEY) {
    return res.status(500).json({ error: 'GEMINI_API_KEY is not configured on the server.' });
  }
  ...
  if (questions.length > AI_TAG_MAX_BATCH) {
    return res.status(400).json({ error: `Batch too large — max ${AI_TAG_MAX_BATCH} questions per request.` });
  }
  ...
  try {
    ...
    catch { return res.status(502).json({ error: 'AI returned unparseable output. Try a smaller batch.' }); }
